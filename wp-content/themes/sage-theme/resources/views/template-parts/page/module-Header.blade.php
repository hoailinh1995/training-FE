<section class="module-Header font-poppins xx:w-full">
    <div class="bg-cover left-0 right-0 top-0 bottom-0 xx:h-943 2xl:h-794 absolute z-1" style="background-image:url(@asset('/images/header.png'))" ></div>
    <!-- <div class="border top-2 absolute z-10"></div> -->
    <div class="container absolute z-2 xx:top-19 xx:left-360 2xl:left-130 2xl:top-19">
        <div class="logo w-30-per">
            <img src="@asset('/images/encompass.png')">
        </div>
        <div class="menu w-52-per row text-left absolute z-2 left-508 top-1 -ml-2">
            <div class="btn-white w-1/4 font-semibold text-lg">
                    ABOUT US
            </div>
            <div class="btn-white w-1/4 font-semibold text-lg">
                    LEADERSHIP
            </div>
            <div class="btn-white w-1/4 font-semibold text-lg pl-6">
                    CAREERS
            </div>
            <div class="btn-white w-1/4 font-semibold text-lg">
                    CONTACT US
            </div>
        </div>
        <div class="title w-40-per xx:mt-180 xx:ml-1 2xl:mt-68 animate-opacity">
            <div class="font-bold">
                <h2 class="text-white text-7xl">
                    Unlock the Power of Connection
                </h2>
            </div> 
            <div class="content xx:w-70-per 2xl:w-70-per text-xl font-light">
                <p class="text-white">
                    Connect & Grow Your Business with the Power of the Encompass Platform
                </p>
            </div>
            <div class="text-base">
                <button class="btn-red font-extrabold rounded-full w-240 h-26">ABOUT US</button>
            </div>
        </div>
        <div class="pic xx:w-60-per">
            <div class="pic absolute w-71-per xx:top-130 xx:-right-173 2xl:-right-152 2xl:top-109"><img src="@asset('/images/lap.png')" alt=""></div>
            <div class="phone absolute xx:top-370 xx:right-430 2xl:right-454 2xl:top-353"><img src="@asset('/images/phone.png')" alt=""></div>
        </div>
    </div>
</section>