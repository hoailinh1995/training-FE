<section class="module-TheLatest font-nunito">
    <meta name="viewport" content="width-device-width, initial-scale=1">
        <div class="container">
            <div class="font-bold text-red-700 p-4">
                <p>THE LATEST</p>
            </div>
            <div class="flex flex-row flex-wrap ">
                <div class="left w-full lg:w-1/2  p-4">
                    <div class=" bg-purple-500 rounded-3xl">
                        <img class="rounded-t-3xl" src="@asset('images/May27.jpg')">
                        <div class="p-8">
                            <div class="font-bold">
                                <h3>
                                    Lorem ipsum dolor sit amet, conseyetur
                                </h3>
                            </div>
                            <div class="font-light">
                                <p>
                                    Lorem ipsum dolor sit amet, conseyetur sadipscing elit, sed diam nonumy eirmod.
                                </p>
                                <div class="font-bold text-red-700">
                                    READ MORE<span class="icomoon icon-chevron-right"></span>
                                </div>
                            </div>
                        </div> 
                    </div>
                </div>
                <div class="right flex-col flex-wrap w-full lg:w-1/2">
                    <div class="up w-full md:w-full p-4">
                        <div class=" rounded-3xl p-8 bg-red-500">
                            <div class="font-bold">
                                <h3>
                                    Lorem ipsum dolor sit amet, conseyetur
                                </h3>
                            </div>
                            <div class="font-light">
                                <p>
                                    Lorem ipsum dolor sit amet, conseyetur sadipscing elit, sed diam nonumy eirmod.
                                </p>
                            </div>
                            <div class="more" style="display: none;">
                                <div class="font-light">
                                    <p>
                                    Lorem ispum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod, sed diam nonumy eirmod Lorem ipsum dolor sit anet consetetur sadipscing elitr, sed diam nonumy eirmod. Lorem ipsum dolir sit amet, consetetur.
                                    </p>
                                </div>
                            </div>
                            <div class="flip font-bold text-red-700">
                                READ MORE<span class="icomoon icon-chevron-right"></span>
                            </div>
                        </div>
                    </div>
                    <div class="down w-full md:w-full p-4">
                        <div class="bg-green-500 rounded-3xl p-8">
                            <div class="font-bold">
                            <h3>
                                Lorem ipsum dolor sit amet, conseyetur
                            </h3>
                        </div>
                        <div class="font-light">
                            <p>
                                Lorem ipsum dolor sit amet, conseyetur sadipscing elit, sed diam nonumy eirmod.
                                <span class="icomoon icon-iconC w-1432 h-675"></span>
                            </p>
                            <div class="font-bold text-red-700">
                                READ MORE<span class="icomoon icon-chevron-right"></span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
</section>
